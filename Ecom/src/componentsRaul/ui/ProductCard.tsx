...
export default function ProductCard(props: ProductCardProps) {
    return (
        <figure className="relative flex flex-col h-90 w-55 p-3 bg-light-gray rounded-2xl shadow-md font-quicksand group">
            {props.edit && <ButtonComponent style="absolute top-4 right-4 w-9 h-9 bg-contrast-main rounded-xl flex items-center cursor-pointer justify-center hover:bg-contrast-secondary hover:text-white transition-all duration-400" icon={<IconEdit />} />}
            {!props.edit &&
                <div className="group-hover:opacity-100 opacity-0 transition-all duration-300 ease-in-out">
                    <ButtonComponent icon={<IconHeart />} iconStyle="text-white" style="absolute top-3 right-3 w-9 h-9 bg-contrast-main rounded-xl flex items-center cursor-pointer justify-center hover:bg-contrast-secondary hover:text-white transition-all duration-400" />
                </div>
            }
